...
if(state<=10){
    ...
    if(state<=10){if(obstacles1.x - mainCharacter.x < displayWidth){
...
if(obstacles4.x - mainCharacter.x < displayWidth){
   ...
   mainCharacter.collide(invisibleGround);
    ...
    if(state===11.5){
        ...
        console.log("in 11.5 ="+monster.y);
        if(monster.x>=mainCharacter.x+displayWidth+300){
            textSize(30);
            text("You Win", mainCharacter.x+400, 300);
            monster.y = -10;
            monster.velocityX = 0;
            win = false;
            if(keyDown('up')){
    ...
    decreaseLife();
    text("life = "+life, mainCharacter.x+100, 300);
    text("state = "+state, mainCharacter.x+100, 200);
    text("Monster = "+monster.y, mainCharacter.x+200, 300);
    text("monsterX = "+monster.x, mainCharacter.x+200, 400);
    text("life state 2 = "+life1, mainCharacter.x+100, 100)
    text("count = "+animeCount, mainCharacter.x+100, 500);

    fill('red')
    text('1 = '+obstacles1.x, mainCharacter.x+500, 100);    
    text('2 = '+obstacles2.x, mainCharacter.x+500, 200);    
    text('3 = '+obstacles3.x, mainCharacter.x+500, 300);    
    text('4 = '+obstacles4.x, mainCharacter.x+500, 400);
    text('Values = '+ mainCharacter.x, mainCharacter.x+500, 500);
    text('Valuesss = '+ obstacles1.x, mainCharacter.x+700, 500);
    moreState();
    console.log(obstacles1.y-mainCharacter.y)
    lose();
}
...
function lose(){
    ...
    if(mainCharacter.y>=displayHeight){
...
function changeState(){
    if(stateistwo === true){
        ...
        text("Velocity = "+obstacles1.velocityX, mainCharacter.x+500, 100);
        text("X = "+obstacles1.x, mainCharacter.x+500, 200);
        text("X2 = "+obstacles2.x, mainCharacter.x+500, 400);
...
function moreState(){
    ...
    if(state===16){
        ...
        if(life>=1040){
            ...
            if(monster.x>=mainCharacter.x+displayWidth){
                textSize(30);
                text("You Recaptured the Kingdom", mainCharacter.x+400, 300);
                monster.y = -10;
                monster.velocityX = 0;
                win = false;
            }
...
function monsterCharge(){
    if(state === 11 || state === 12 || state === 13 || state === 14 || state === 15){
        if(monster.x-mainCharacter.x<250 && monster.x-mainCharacter.x>0){
            monster.velocityX = -4;
        }
        if(monster.x-mainCharacter.x<0){
            monster.velocityX = 4;
        }
    }
}
